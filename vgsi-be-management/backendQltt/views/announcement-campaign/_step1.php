...
$listTemplate = AnnouncementTemplate::find()->where(['type' => AnnouncementTemplate::TYPE_POST_NEWS])->orderBy(['created_at' => SORT_DESC])->all();
$templateNames = \yii\helpers\ArrayHelper::map($listTemplate, 'id', (isset($_COOKIE['language']) && $_COOKIE['language'] == 'en') ? 'name_en' : 'name');
...
<div class="tab-pane active" role="tabpanel" id="step1">
    <div class="row">
        ...
        <?= $form->field($model, 'id')->dropDownList($templateNames, [
            'prompt' => 'Select a template',
            // Tuỳ chọn mặc định cho dropdown
            'id' => 'news_template', // Đặt id cho dropdown
        ])->label(""); ?>


        <?= $form->field($model, 'content')->widget(CKEditor::className(), [
            'id' => 'contentEmailCampaingn',
            // Đặt ID cho trường CKEditor
            'options' => ['rows' => 6],
            'preset' => 'basic',
            'class' => 'contentEmailCampaingn'
        ...
        <div class="form-group highlight-addon field-status">
            ...
            <div class="col-md-10 d-flex">
                <?php
                $className = ($model->status == AnnouncementCampaign::STATUS_PUBLIC_AT || $model->send_event_at >= time()) ? '' : 'd-none';
                $publicAt = $model->send_event_at ? date('Y-m-d\TH:i', $model->send_event_at) : date('Y-m-d\TH:i', time());
                ?>
                <?= Html::dropDownList('AnnouncementCampaign[status]', is_numeric($model->status) ? ($model->status === AnnouncementCampaign::STATUS_UNACTIVE && $model->send_event_at >= time() ? AnnouncementCampaign::STATUS_PUBLIC_AT : $model->status) : AnnouncementCampaign::STATUS_ACTIVE, $statusList, ['class' => 'form-control', 'style' => 'max-width: 210px;', 'id' => 'status', 'disabled' => $model->status === AnnouncementCampaign::STATUS_ACTIVE || ($model->status === AnnouncementCampaign::STATUS_PUBLIC_AT && $model->send_event_at < time())]); ?>
    ...
    <ul class="list-inline text-center">
        ...
        <?php } ?>
        <?php $isTrash = Yii::$app->request->get('isTrash', null);
        if ($isTrash == null) {
            if ($model->status == AnnouncementCampaign::STATUS_PUBLIC_AT || $model->status == AnnouncementCampaign::STATUS_ACTIVE) { ?>
            <li>
                <button type="button" id="next-step" class="btn btn-primary skip-btn">
