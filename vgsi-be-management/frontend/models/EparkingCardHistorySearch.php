...
class EparkingCardHistorySearch extends EparkingCardHistory
{
    ...
    {
        return [
            [['id', 'vehicle_type', 'card_type', 'ticket_type', 'datetime_in', 'datetime_out', 'status', 'created_at', 'updated_at', 'created_by', 'updated_by', 'service_management_vehicle_id', 'building_cluster_id', 'apartment_id', 'start_datetime', 'end_datetime'], 'integer'],
    ...
    public function search($params)
    {
        ...
        $query = EparkingCardHistoryResponse::find()->where(['building_cluster_id' => $buildingCluster->id]);
...
//            'building_cluster_id' => $this->building_cluster_id,
        ...
        if(!empty($this->start_datetime)){
            $query->where(['>=', 'created_at', $this->start_datetime]);
        }
        if(!empty($this->end_datetime)){
            $query->where(['<=', 'created_at', $this->end_datetime]);
        }

        $query->andFilterWhere(['like', 'serial', $this->number])
