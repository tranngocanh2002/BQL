...
class ServicePaymentFeeResponse extends ServicePaymentFee
{
     ...
     * @SWG\Property(property="is_pay", type="integer", description="0 - không được thanh toán nữa, 1 -được thanh toán tiếp, 2 - được phép thanh toán kèm gen code gửi lên để tạo bill"),
     * @SWG\Property(property="service_bill_invoice_id", type="integer", description="id phiếu chi nếu đã tạo phiếu chi thì sẽ != null"),
                    ...
                    if($model->type == ServicePaymentFee::TYPE_SERVICE_PARKING_FEE){
                        ...
                        if($model->type === ServicePaymentFee::TYPE_SERVICE_BOOKING_FEE)
                        {
                            //nếu phí từ book sẽ lấy thêm thông tin tiện ích
                             $booking = ServiceUtilityBooking::find()->where(['like', 'service_payment_fee_ids_text_search', ','.$model->id.','])->one();
                            if(!empty($booking)){
...
//                    $name = 'Phát sinh - ';
                ...
                if (!empty($model->serviceMapManagement)) {
                    $name .= $model->serviceMapManagement->service_name_en;
                    if($model->type == ServicePaymentFee::TYPE_SERVICE_PARKING_FEE){
                        $serviceParkingFee = ServiceParkingFee::findOne(['service_payment_fee_id' => $model->id]);
                        if(!empty($serviceParkingFee)){
                            if(!empty($serviceParkingFee->serviceManagementVehicle)){
                        ...
                        if($model->type === ServicePaymentFee::TYPE_SERVICE_BOOKING_FEE)
                        {
                            ...
                            if(!empty($booking)){
                                if(!empty($booking->serviceUtilityFree)){
                                    $name .= ' - ' .$booking->serviceUtilityFree->name_en;
                                }
                            }
                        }
                    }
