 ...
 * @SWG\Definition(
 *   type="object",
 *   @SWG\Xml(name="ServiceWaterFeeImportForm")
 * )
 */
class ServiceWaterFeeImportForm extends Model
{
    /**
...
//        $xls_datas = $spreadsheet->getActiveSheet()->toArray(null, true, true, true);
        ...
        while (true) {
            $rows = [];
            $stop = 0;
            $setSheet = 'Danh sách chốt số nước';
            ...
            if($rows[2] > $rows[4]){
                $apartmentMapResidentUserApprovedError[] = [
                    'line' => $i - 2,
                    'apartment_name' => $rows[1],
                    'apartment_parent_path' => $rows[3].'/'.$rows[5],
            ...
            if (empty($apartmentMapResidentUser)) {
                ...
                if ($is_validate == 0) {
                    ...
                    $serviceWaterInfo = ServiceWaterInfo::findOne(['apartment_id' => $apartmentMapResidentUser->apartment_id, 'service_map_management_id' => $service_map_management_id]);
                    if(empty($serviceWaterInfo)){
                        $serviceWaterInfoError[] = [
                            'line' => $i - 2,
                            'apartment_name' => $rows[1],
                            'apartment_parent_path' => $rows[2].'/'.$rows[3],
                            'message' => Yii::t('frontend', "Chỉ số sử dụng ban đầu đã được khai báo")
                        ];
                        continue;
                    }

                    if($lock_time < $serviceWaterInfo->tmp_end_date){
                        $serviceWaterInfoError[] = [
                            'line' => $i - 2,
                            'apartment_name' => $rows[1],
                            'apartment_parent_path' => $rows[2].'/'.$rows[3],
                    ...
                    $start_index = $serviceWaterInfo->end_index;
                    ...
                    if (!empty($serviceWater)) {
                        if($serviceWater->status == ServiceWaterFee::STATUS_UNACTIVE){
                            $apartmentMapResidentUserApprovedError[] = [
                                'line' => $i - 2,
                                'apartment_name' => $rows[1],
                                'apartment_parent_path' => $rows[2].'/'.$rows[3],
                                'message' => Yii::t('frontend', "Đang có phí nước chưa được duyệt")
                            ];
                            continue;
                        }
                    }
                    ...
                    if(isset($rows[3]) && (trim($rows[3]) != '') && $rows[3] >= 0){
                    ...
                    if ($start_index >= $rows[3] || ($rows[3] <= 0 && $rows[5] <= 0) || $rows[3] < 0 || (isset($rows[5]) && (trim($rows[5]) != '') && $rows[5] < 0)) {
                        $arrIndexError[] = [
                            ...
                            'end_index' => $rows[3],
                            'message' => Yii::t('frontend', "Chỉ số sử dụng không phù hợp")
                        ];
                    } else {
//                        $serviceWaterFeeOld = ServiceWaterFee::find()->where(['building_cluster_id' => $building_cluster_id, 'apartment_id' => $apartmentMapResidentUser->apartment_id])->orderBy(['end_index' => SORT_DESC])->one();
//                        if (!empty($serviceWaterFeeOld)) {
//                            $start_time = $serviceWaterFeeOld->lock_time;
//                        } else {
...
//                        }
                        ...
                        $serviceWaterFee->start_index = $start_index;
                        $serviceWaterFee->end_index = $rows[3];
                        $serviceWaterFee->total_index = $rows[3] - $start_index;
                        $genchage = $serviceWaterFee->getCharge($building_cluster_id, $service_map_management_id, $rows[3], $start_index, $lock_time,$apartmentMapResidentUser->apartment_id);
                        ...
                        $serviceWaterFee->json_desc = json_encode($genchage['json_desc']);
...
//            $success = false;
    ...
    public function genForm()
    {
        $buildingCluster = Yii::$app->building->BuildingCluster;
        $building_cluster_id = $buildingCluster->id;
        $file_path = '/uploads/fee/'.time().'-service-water-fee.xlsx';
        $fileandpath = \Yii::getAlias('@webroot') . $file_path;
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Danh sách chốt số nước');
...
//            $sheet->setCellValue('F'.$i, "'".date('d/m/Y'));
        ...
        //  $sheet2->getStyle('A3:C3')->applyFromArray($wrapStyle);
         ...
         $sheet2->setCellValue('A5','Số chốt cuối');
