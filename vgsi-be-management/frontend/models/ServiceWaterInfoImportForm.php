 ...
 * @SWG\Definition(
 *   type="object",
 *   @SWG\Xml(name="ServiceWaterInfoImportForm")
 * )
 */
class ServiceWaterInfoImportForm extends Model
{
    /**
...
//        $xls_datas = $spreadsheet->getActiveSheet()->toArray(null, true, true, true);
        ...
        while (true) {
            $rows = [];
            $stop = 0;
            $setSheet = 'Danh sách sử dụng nước';
            ...
            if (empty($apartmentMapResidentUser)) {
                ...
                if ($is_validate == 0) {
                    $ServiceWaterInfo = ServiceWaterInfo::findOne(['building_cluster_id' => $building_cluster_id, 'apartment_id' => $apartmentMapResidentUser->apartment_id, 'service_map_management_id' => $service_map_management_id]);
                    if(!empty($ServiceWaterInfo)){
                        $ServiceWaterInfoError[] = [
                            'line' => $i - 2,
                            'apartment_name' => $rows[1],
                            'apartment_parent_path' => $rows[2].'/'.$rows[3],
                            'message' => Yii::t('frontend', "Bất động sản đã được khai báo sử dụng nước")
                        ];
                        continue;
                    }

                    $ServiceWaterInfo = new ServiceWaterInfo();
                    $ServiceWaterInfo->building_cluster_id = $apartmentMapResidentUser->building_cluster_id;
                    $ServiceWaterInfo->building_area_id = $apartmentMapResidentUser->building_area_id;
                    $ServiceWaterInfo->apartment_id = $apartmentMapResidentUser->apartment_id;
                    $ServiceWaterInfo->service_map_management_id = $service_map_management_id;
                    $ServiceWaterInfo->start_date = $rows[2];
                    $ServiceWaterInfo->end_date = $rows[3];
                    $ServiceWaterInfo->tmp_end_date = $rows[3];
                    $ServiceWaterInfo->end_index = (int)$rows[4];
                    if (!$ServiceWaterInfo->save()) {
                        ...
                        $arrCreateError[] = [
                            ...
                            'apartment_name' => $rows[1],
                            'apartment_parent_path' => $rows[2].'/'.$rows[3],
                            'errors' => $ServiceWaterInfo->errors,
                            'message' => Yii::t('frontend', "Khai báo phí sử dụng ban đầu không thành công")
                        ];
                    }else{
                        $imported++;
                    }
    ...
    public function genForm()
    {
        $buildingCluster = Yii::$app->building->BuildingCluster;
        $building_cluster_id = $buildingCluster->id;
        $file_path = '/uploads/fee/' . time() . '-service-water-info.xlsx';
        $fileandpath = \Yii::getAlias('@webroot') . $file_path;
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Danh sách sử dụng nước');
