...
$html = str_replace('{{DAY/MONTH/YEAR}}', date('d/m/Y', time()), $html);
$html = str_replace('{{MONTH/YEAR}}', date('m/Y', time()), $html);
...
?>
<?php
$buildingFeeList = $this->render('table/buidling_fee_list', ['serviceBuildingConfig' => $serviceBuildingConfig, 'total_servicePaymentFee' => $total_servicePaymentFee, 'servicePaymentFee' => $servicePaymentFee, 'apartment' => $apartment, 'i_count' => $i_count]);
$html = str_replace('{{BUILDING_FEE_LIST}}', $buildingFeeList, $html);
if(!empty($buildingFeeList)){ $i_count++;}
?>
<?php
$waterFeeList = $this->render('table/water_fee_list', ['serviceBuildingConfig' => $serviceBuildingConfig, 'servicePaymentFeeWater' => $servicePaymentFeeWater, 'total_servicePaymentFeeWater' => $total_servicePaymentFeeWater, 'apartment' => $apartment, 'i_count' => $i_count]);
$html = str_replace('{{WATER_FEE_LIST}}', $waterFeeList, $html);
if(!empty($waterFeeList)){ $i_count++;}
?>
<?php
