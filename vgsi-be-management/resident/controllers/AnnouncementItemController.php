...
class AnnouncementItemController extends ApiController
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $announce_category_search = new AnnouncementItemSearch();
        $dataProvider = $announce_category_search->search(Yii::$app->request->queryParams);
        return [
    ...
    {
        $announcementItem = AnnouncementItemResponse::findOne(['id' => (int)$id]);
        if (empty($announcementItem->read_at)) {
            $announcementItem->read_at = time();
            $announcementItem->save();
            $announcementCampaign = AnnouncementCampaign::findOne(['id' => $announcementItem->announcement_campaign_id]);
            $announcementCampaign->total_apartment_open++;
            if($announcementCampaign->total_app_open < $announcementCampaign->total_app_send){
                $announcementCampaign->total_app_open++;
            }
            $announcementCampaign->save();
        }
