 ...
 * @SWG\Definition(
 *   type="object",
 *   @SWG\Xml(name="ResidentUserNotifyIsHiddenForm")
 * )
 */
class ResidentUserNotifyIsHiddenForm extends Model
{
    ...
    public function isHidden()
    {
        try {
            ...
            if ($this->is_hidden_all == self::IS_HIDDEN_ALL) {
                ResidentUserNotify::updateAll(['is_hidden' => ResidentUserNotify::IS_HIDDEN], ['resident_user_id' => $user->id, 'is_hidden' => ResidentUserNotify::IS_NOT_HIDDEN]);
            } else {
                if (!empty($this->is_hidden_array) && is_array($this->is_hidden_array)) {
                    ResidentUserNotify::updateAll(['is_hidden' => ResidentUserNotify::IS_HIDDEN], ['id' => $this->is_hidden_array, 'resident_user_id' => $user->id, 'is_hidden' => ResidentUserNotify::IS_NOT_HIDDEN]);
                }
            }

            if ($this->is_not_hidden_all == self::IS_NOT_HIDDEN_ALL) {
                ResidentUserNotify::updateAll(['is_hidden' => ResidentUserNotify::IS_NOT_HIDDEN], ['resident_user_id' => $user->id, 'is_hidden' => ResidentUserNotify::IS_HIDDEN]);
            } else {
                if (!empty($this->is_not_hidden_array) && is_array($this->is_not_hidden_array)) {
                    ResidentUserNotify::updateAll(['is_hidden' => ResidentUserNotify::IS_NOT_HIDDEN], ['id' => $this->is_not_hidden_array, 'resident_user_id' => $user->id, 'is_hidden' => ResidentUserNotify::IS_HIDDEN]);
                }
            }
            return [
                'success' => true,
