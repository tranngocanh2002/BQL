...
import makeSelectDashboardBillDetail from "./selectors";
...
export class DashboardBillDetail extends React.PureComponent {
  state = {
  ...
  componentWillReceiveProps(nextProps) {
    // if param id change then fetch new data
    if (this.props.match.params.id != nextProps.match.params.id) {
      this.props.dispatch(fetchDetailBill({ id: nextProps.match.params.id }));
    }

    if (
      (this.props.dashboardBillDetail.loading !=
        nextProps.dashboardBillDetail.loading &&
        !nextProps.dashboardBillDetail.loading &&
        !!nextProps.dashboardBillDetail.data) ||
      (this.props.dashboardBillDetail.updating !=
        nextProps.dashboardBillDetail.updating &&
        !nextProps.dashboardBillDetail.updating &&
        !!nextProps.dashboardBillDetail.data)
    ) {
      const {
      ...
      this.setState({
        ...
        fees: service_payment_fees.map((rr) => {
          return {
            ...rr,
            new_more_money_collecte: rr.more_money_collecte,
            old_money_collected: rr.money_collected,
            new_money_collected: rr.service_bill_item,
          };
        }),
        moneyReality: _.sumBy(
          service_payment_fees,
          (rr) => rr.service_bill_item
        ),
      });
    }
  }
  ...
  _renderInfo = () => {
    const { dashboardBillDetail } = this.props;
    const { canceling, changingStatus, data, updating } = dashboardBillDetail;
    const { isEditting, moneyReality, fees } = this.state;
    ...
    const columns = [
      ...
      {
        // width: 80,
        title: <span>{formatMessage(messages.month)}</span>,
        dataIndex: "fee_month",
        key: "fee_month",
        render: (text, record) => (
      ...
      {
        title: <span>{formatMessage(messages.serviceType)}</span>,
        dataIndex:
        ...
        key:
          this.props.language === "en"
            ? "service_map_management_service_name_en"
            : "service_map_management_service_name",
        // render: (text, record) => <span>{moment.unix(record.fee_of_month).format("MM")}</span>
      },
      {
        // width: 90,
        align: "right",
        title: <span>{formatMessage(messages.receivables)}</span>,
        dataIndex: "price",
        key: "price",
        render: (text, record) => <span>{formatPrice(text)} đ</span>,
      },
      {
        // width: 90,
        align: "right",
        title: <span>{formatMessage(messages.received)}</span>,
        dataIndex: "old_money_collected",
        key: "old_money_collected",
        render: (text, record) => {
          return <span>{formatPrice(text)} đ&ensp;</span>;
        },
      },
      {
        // width: 90,
        align: "right",
        title: <span>{formatMessage(messages.owed)}</span>,
      ...
      },
      {
        // width: 90,
        align: "right",
        title: <span>{formatMessage(messages.realReceiving)}</span>,
        dataIndex: "new_money_collected",
        key: "new_money_collected",
        render: (text, record) => (
          <span style={{ color: "#30A674" }}>{formatPrice(text)} đ</span>
        ),
      },
    ];
    return (
      <Col span={12} style={{ paddingRight: 8 }}>
              ...
              <Row style={{ marginBottom: 16 }}>
                {data.status != 10 && !isEditting && (
                  <Button
                    ...
                    onClick={() => {
                      const { service_payment_fees } =
                        this.props.dashboardBillDetail.data;
                      this.setState({
                        isEditting: true,
                        // fees: service_payment_fees.map(rr => {
                        //   return {
                        ...
                        //     old_money_collected: rr.money_collected - rr.service_bill_item,
                        //     new_money_collected: rr.service_bill_item + rr.more_money_collecte,
                        //   }
                        // }),
                        // moneyReality: _.sumBy(service_payment_fees, rr => rr.service_bill_item + rr.more_money_collecte)
                      });
                    }}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (total_new_money_collected == 0) {
                        notificationBar(
                          formatMessage(messages.errorContent),
                          "warning"
                        );
                        return;
                      }
                      if (total_new_money_collected > moneyReality) {
                        notificationBar(
                          formatMessage(messages.errorContent2),
                          "warning"
                        );
                        return;
                      }
                      this.props.dispatch(
                        updateBill({
                          ...
                          payment_date: this.state.payment_date.unix(),
                          execution_date: this.state.execution_date.unix(),
                        })
                      );
                    }}
                    disabled={!this.state.payer_name.trim().length}
                    type="primary"
                    loading={updating}
                  >
                ...
                {data.status == 1 && !isEditting && (
                  <WithRole roles={[config.ALL_ROLE_NAME.FINANCE_SPECIAL_BILL]}>
                    <Button
                      style={{ marginRight: 10 }}
                      disabled={canceling}
                      ...
                      onClick={() => {
                        Modal.confirm({
                          ...
                          title: formatMessage(messages.modalContent),
                ...
                {data.status != 10 && !isEditting && (
                  <WithRole roles={[config.ALL_ROLE_NAME.FINANCE_SPECIAL_BILL]}>
                    <Button
                      ...
                      onClick={() => {
                        // this.props.dispatch(cancelBill({ id: data.id }))
                        this.setState({
                          visibleConfirm: true,
                          noteFor: "cancel",
                          note: "",
                    ...
                    >
                      {formatMessage(messages.cancelVote)}
                    </Button>
                  </WithRole>
                )}
              </Row>
        ...
        <Row gutter={24}>
          <Col
            span={24}
            style={{ paddingLeft: 16, paddingRight: 32, marginBottom: 16 }}
          >
            <Row type="flex" align="middle">
              <Col span={5} style={{ textAlign: "left" }}>
                {formatMessage(messages.voteNum)}: &ensp;
              </Col>
              <Col
                span={18}
          ...
          <Col span={12} style={{ marginBottom: 16, paddingLeft: 16 }}>
            <Row type="flex" align="middle">
              <Col span={10} style={{ textAlign: "left" }}>
                {formatMessage(messages.property)}: &ensp;
              </Col>
              <Col span={14}>
                <Input
          ...
          <Col span={12} style={{ marginBottom: 16, paddingLeft: 16 }}>
            <Row type="flex" align="middle">
              <Col span={10} style={{ textAlign: "left" }}>
                {formatMessage(messages.status)}: &ensp;
              </Col>
              <Col span={14}>
                <Input
                  value={
          ...
          <Col span={12} style={{ marginBottom: 16, paddingLeft: 16 }}>
            <Row type="flex" align="middle">
              <Col span={10} style={{ textAlign: "left" }}>
                {formatMessage(messages.collector)}: &ensp;
              </Col>
              <Col span={14}>
                <Input
          ...
          <Col span={12} style={{ marginBottom: 16, paddingLeft: 16 }}>
            <Row type="flex" align="middle">
              <Col span={10} style={{ textAlign: "left" }}>
                {formatMessage(messages.submitter)}: &ensp;
              </Col>
              <Col span={14}>
                <Input
          ...
          <Col span={12} style={{ marginBottom: 16, paddingLeft: 16 }}>
            <Row type="flex" align="middle">
              <Col span={10} style={{ textAlign: "left" }}>
                {formatMessage(messages.fillDate)}: &ensp;
              </Col>
              <Col span={14}>
                <DatePicker
          ...
          <Col span={12} style={{ marginBottom: 16, paddingLeft: 16 }}>
            <Row type="flex" align="middle">
              ...
              >
                {formatMessage(messages.implementDate)}: &ensp;
              </Col>
              <Col span={14}>
                <DatePicker
                  allowClear={isEditting ? false : true}
          ...
          <Col span={12} style={{ marginBottom: 16, paddingLeft: 16 }}>
            <Row type="flex" align="middle">
              <Col span={10} style={{ textAlign: "left" }}>
                {formatMessage(messages.submissForm)}: &ensp;
              </Col>
              <Col span={14}>
                <Select
          ...
          <Col span={12} style={{ marginBottom: 16, paddingLeft: 16 }}>
            <Row type="flex" align="middle">
              <Col span={10} style={{ textAlign: "left" }}>
                {formatMessage(messages.realReceiving)}: &ensp;
              </Col>
              <Col span={14}>
                <InputNumberFormat
                  disabled={true}
  ...
  _renderPhieuThu = () => {
    const { dashboardBillDetail, buildingCluster, language } = this.props;
    const { data } = dashboardBillDetail;
      ...
      fees: parseBillToView(
        ...
        if (
          ...
          return (
            ...
            //     rr,
            //   }}
            //   jsx={service_bill_templateJSON.jsx_row}
            // />
  ...
  render() {
    const { dashboardBillDetail } = this.props;
    const { loading, data, canceling, cancelSuccess } = dashboardBillDetail;
    ...
    if (!data) {
      return (
        <Page inner>
          <Exception
            type="404"
            desc={this.props.intl.formatMessage(messages.notFound)}
            actions={
              <Button
                type="primary"
                onClick={() => this.props.history.push("/main/finance/bills")}
              >
    ...
    return (
      <Page inner>
        <Row className="dashboardBillDetail">
          <Col span={24} style={{ marginBottom: 16 }}>
            <Row type="flex" align="middle">
              ...
              >
                ...
                &ensp;{this.props.intl.formatMessage(messages.back)}
              </a>
            </Row>
          </Col>
          <Col span={24}>
          ...
          <Modal
            ...
            title={
              noteFor == "cancel"
                ? this.props.intl.formatMessage(messages.rejectReceipt)
                : noteFor == "unlock"
                ? this.props.intl.formatMessage(messages.unlockReceipt)
                : ""
            }
          ...
          >
            <Row>
              <Col span={4} style={{ textAlign: "right" }}>
                <span>
                  {this.props.intl.formatMessage(messages.reason)}:&ensp;
                </span>
              </Col>
              <Col span={20}>
                <Input.TextArea
...
DashboardBillDetail.propTypes = {
  dispatch: PropTypes.func.isRequired,
};

const mapStateToProps = createStructuredSelector({
  dashboardBillDetail: makeSelectDashboardBillDetail(),
...
const withReducer = injectReducer({ key: "dashboardBillDetail", reducer });
const withSaga = injectSaga({ key: "dashboardBillDetail", saga });

export default compose(
  withReducer,
  withSaga,
  withConnect
)(hotkeys(injectIntl(DashboardBillDetail)));
