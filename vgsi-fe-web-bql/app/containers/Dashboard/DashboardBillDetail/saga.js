...
function* _cancelBill(action) {
  try {
    ...
    if (res.success) {
      if (language === "en") {
        notificationBar("Cancel the receipt successful.");
      } else {
        notificationBar("Hủy phiếu thu thành công.");
      }
...
function* _updateBill(action) {
  try {
    let res = yield window.connection.updateBill(action.payload);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
        notificationBar("Update the bill successful.");
      } else {
        notificationBar("Cập nhật phiếu thanh toán thành công.");
      }
