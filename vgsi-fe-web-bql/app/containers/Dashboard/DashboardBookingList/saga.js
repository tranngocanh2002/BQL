...
function* _fetchBookingList(action) {
  try {
    let res = yield window.connection.fetchBookingUtility({
      ...action.payload,
      pageSize: 20,
    });
    if (res.success) {
...
function* _createBooking(action) {
  try {
    ...
    let res = yield window.connection.createBookingUtility(rest);
...
export default function* rootSaga() {
  yield all([
    takeLatest(FETCH_ALL_BOOKING, _fetchBookingList),
