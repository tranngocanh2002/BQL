...
function* _createOrder(action) {
  try {
    ...
    let res = yield window.connection.createInvoiceBill(rest);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
        notificationBar("Create payment voucher successful.");
      } else {
        notificationBar("Lập phiếu chi thành công.");
      }
          ...
          fetchFeeOfApartmentComplete({
            items: res.data.items.filter((iii) => {
              ...
              return (
                ...
                (iii.money_collected != 0 && !iii.service_bill_invoice_id)
              );
            }),
