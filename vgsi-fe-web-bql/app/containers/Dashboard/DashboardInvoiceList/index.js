...
import makeSelectDashboardInvoiceList from "./selectors";
...
export class DashboardInvoiceList extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  handleTableChange = (pagination, filters, sorter) => {
    ...
    this.setState({ sort, current: pagination.current }, () => {
      this.props.history.push(
        `/main/finance/invoice-bill?${queryString.stringify({
          ...this.state.filter,
  ...
  render() {
    const { dashboardInvoiceList } = this.props;
    ...
    let columns = [
      ...
      {
        title: <span className="nameTable">Thao tác</span>,
        dataIndex: "iddd8",
        key: "iddd8",
        align: "center",
        render: (text, record) => (
          <Row>
            <Tooltip title={formatMessage(messages.detail)}>
              <Link to={`/main/finance/invoice-bill/bill/${record.id}`}>
                <Icon type="container" />
              </Link>
            </Tooltip>
          </Row>
    ...
    return (
      <>
        ...
        <Page inner className="DashboardInvoiceList">
          <Row>
            ...
            <Col span={24}>
              <Table
                ...
                pagination={{
                  pageSize: 20,
                  total: dashboardInvoiceList.totalPage,
                ...
                onRow={(record) => {
                  return {
                    onClick: () => {
                      this.props.history.push(
                        `/main/finance/invoice-bill/bill/${record.id}`
                      );
                    }, // click row
                  };
                }}
...
const mapStateToProps = createStructuredSelector({
  dashboardInvoiceList: makeSelectDashboardInvoiceList(),
});
...
const withReducer = injectReducer({ key: "dashboardInvoiceList", reducer });
const withSaga = injectSaga({ key: "dashboardInvoiceList", saga });

export default compose(
  withReducer,
  withSaga,
  withConnect
)(injectIntl(withRouter(DashboardInvoiceList)));
