...
import makeSelectBillDetail from "./selectors";
...
export class BillDetail extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.billDetail.detail.loading !==
        nextProps.billDetail.detail.loading &&
      !nextProps.billDetail.detail.loading
    ) {
      this.setState({
        record: nextProps.billDetail.detail.data,
        totalMoney: nextProps.billDetail.detail.data.service_bill_items.reduce(
          (total, fee) => {
  ...
  handleDelete = () => {
    ...
    Modal.confirm({
      autoFocusButton: null,
      title: this.props.intl.formatMessage(messages.content3),
      okText: this.props.intl.formatMessage(messages.agree),
      okType: "danger",
      cancelText: this.props.intl.formatMessage(messages.cancel),
      onOk: () => {
  ...
  render() {
    const { billDetail, intl } = this.props;
    const { detail, updating } = billDetail;
...
const mapStateToProps = createStructuredSelector({
  billDetail: makeSelectBillDetail(),
});
...
const withReducer = injectReducer({ key: "billDetail", reducer });
const withSaga = injectSaga({ key: "billDetail", saga });

export default compose(
  withReducer,
  withSaga,
  withConnect
)(injectIntl(BillDetail));
