...
import {
  createNotificationUpdateReminder,
  defaultAction,
...
import { emailReg } from "../NotificationAdd/constants";
...
export class NotificationUpdate extends React.PureComponent {
  constructor(props) {
    ...
    const notiType = params.notiType;
  ...
  componentDidMount() {
    ...
    this.props.dispatch(
      fetchCategory({
        type: this.state.notiType == "6" ? 2 : 0,
        pageSize: 100,
      })
    );
    this.props.dispatch(fetchBuildingAreaAction());
    this.props.dispatch(
      fetchAllAnnouncementFeeTemplate({ type: this.state.notiType })
    );
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      !nextProps.notificationUpdate.buildingArea.loading &&
      !!this.props.notificationUpdate.detail &&
      this.props.notificationUpdate.detail.status !== 1
    ) {
      this.setState({
        treeData: parseTree(
    ...
    if (
      this.props.notificationUpdate.template.loading !=
        nextProps.notificationUpdate.template.loading &&
      !nextProps.notificationUpdate.template.loading
    ) {
      const { setFields } = this.props.form;
      if (nextProps.notificationUpdate.template.data) {
        let blockArray = htmlToDraft(
          nextProps.notificationUpdate.template.data.content_email
        );
        setFields({
          title: {
            value: nextProps.notificationUpdate.template.data.name,
          },
          title_en: {
            value: nextProps.notificationUpdate.template.data.name_en,
          },
          // content_sms: {
          //   value: nextProps.notificationUpdate.template.data.content_sms,
          // },
        });
        this.setState({
          editorState:
    ...
    if (
      ...
      let pushType = [];
      if (nextProps.notificationUpdate.detail.is_send_email == 1) {
        pushType.push("is_send_email");
      }
      if (nextProps.notificationUpdate.detail.is_send_push == 1) {
        pushType.push("is_send_push");
      }
      // if (nextProps.notificationUpdate.detail.is_send_sms == 1) {
      //   pushType.push("is_send_sms");
      // }
      const { setFields } = this.props.form;
      let blockArray = htmlToDraft(
        nextProps.notificationUpdate.detail.content
          .replaceAll(
            "https://drive.google.com/file/d/",
            "https://drive.google.com/uc?export=view&id="
          )
      ...
      // });
      if (
        nextProps.notificationUpdate.detail.targets &&
        nextProps.notificationUpdate.detail.targets.length > 0
      ) {
        this.setState({
          targets: nextProps.notificationUpdate.detail.targets,
        });
      }
      if (
        nextProps.notificationUpdate.detail.resident_user_phones &&
        nextProps.notificationUpdate.detail.resident_user_phones.length > 0
      ) {
        this.setState({
          resident_user_phones:
            nextProps.notificationUpdate.detail.resident_user_phones,
      ...
      if (
        nextProps.notificationUpdate.detail.survey_deadline &&
        nextProps.notificationUpdate.detail.survey_deadline < moment().unix()
      ) {
        this.setState({
          is_deadline: true,
        });
      }
      if (nextProps.notificationUpdate.detail.survey_deadline) {
        this.setState({
          survey_deadline: moment.unix(
            nextProps.notificationUpdate.detail.survey_deadline
          ),
        });
      }
      let is_send_at = 1;
      let currentTimeSent = moment.unix(
        nextProps.notificationUpdate.detail.send_at
      );
      if (
        nextProps.notificationUpdate.detail.status == 0 &&
        nextProps.notificationUpdate.detail.send_at < moment().unix()
      ) {
        is_send_at = 0;
        currentTimeSent = moment();
      }
      this.setState({
        pushType,
        prevType:
          nextProps.notificationUpdate.detail.is_send_push == 1
            ? 1
            : nextProps.notificationUpdate.detail.is_send_email == 1
            ? 0
            : 2,
        editorState:
          !!blockArray && !!blockArray.contentBlocks
        ...
        fileList:
          ...
          !!nextProps.notificationUpdate.detail.attach.fileList
            ? nextProps.notificationUpdate.detail.attach.fileList
            : [],
        buildingAreaList: nextProps.notificationUpdate.detail.building_area_ids,
        is_event: nextProps.notificationUpdate.detail.is_event == 1,
        is_send_at: is_send_at,
        is_survey: nextProps.notificationUpdate.detail.is_survey == 1,
        formula: nextProps.notificationUpdate.detail.type_report,
        survey_deadline: nextProps.notificationUpdate.detail.survey_deadline
          ? moment.unix(nextProps.notificationUpdate.detail.survey_deadline)
          : moment(),
        currentTimeSent: currentTimeSent,
        send_event_at: nextProps.notificationUpdate.detail.send_event_at
          ? moment.unix(nextProps.notificationUpdate.detail.send_event_at)
          : moment(),
        phone_list: nextProps.notificationUpdate.detail.add_phone_send,
        email_list: nextProps.notificationUpdate.detail.add_email_send,
      });
    }

    if (
      this.props.notificationUpdate.apartmentToSend.loading !=
        nextProps.notificationUpdate.apartmentToSend.loading &&
      !nextProps.notificationUpdate.apartmentToSend.loading &&
      !!nextProps.notificationUpdate.detail &&
      nextProps.notificationUpdate.detail.status == 0 &&
      !!nextProps.notificationUpdate.apartmentToSend.data.length
    ) {
      const phone_not_send = this.state.resident_user_phones || [];
      if (
        !this.state.reChangeTree &&
        !!(
          nextProps.notificationUpdate.detail.apartment_ids &&
          nextProps.notificationUpdate.detail.apartment_ids.length +
            nextProps.notificationUpdate.detail.apartment_not_send_ids.length
        )
      ) {
        if (nextProps.notificationUpdate.detail.apartment_ids.length) {
          // console.log("0");

          this.setState({
            apartment_not_send_ids:
              nextProps.notificationUpdate.apartmentToSend.data
                .filter(
                  (row) =>
                    !nextProps.notificationUpdate.detail.apartment_ids.includes(
                      row.apartment_id
                    )
                )
                .map((opt) => opt.apartment_id),

            apartment_ids: nextProps.notificationUpdate.detail.apartment_ids,
            selectedRow: nextProps.notificationUpdate.apartmentToSend.data
              .filter(
                (row) =>
                  nextProps.notificationUpdate.detail.apartment_ids.includes(
                    row.apartment_id
          ...
          this.setState({
            apartment_not_send_ids:
              nextProps.notificationUpdate.detail.apartment_not_send_ids,
            apartment_ids: nextProps.notificationUpdate.apartmentToSend.data
              .filter(
                (row) =>
                  !nextProps.notificationUpdate.detail.apartment_not_send_ids.includes(
                    row.apartment_id
                  )
              )
              .map((opt) => opt.apartment_id)
        ...
        let newSelect = nextProps.notificationUpdate.apartmentToSend.data
          .map((row) => {
            return row.apartment_id;
          })
          .filter((value, index, self) => self.indexOf(value) === index);
        this.setState({
          apartment_ids: newSelect,
          apartment_not_send_ids: [],
          selectedRow: nextProps.notificationUpdate.apartmentToSend.data
            .filter((row) => !phone_not_send.includes(row.phone))
            .map((opt) => opt.id),
    ...
    if (
      ...
      this.setState({
        dataSource: nextProps.notificationUpdate.apartmentToSend.data,
      });
    }
  }
  getApartmentIds = () => {
    ...
    if (
      apartment_ids.length === notificationUpdate.apartmentToSend.data.length
    ) {
      return [];
    }
  ...
  handleOk = (status, message) => {
    ...
    validateFieldsAndScroll((errors, values) => {
      const { status: real } = this.props.notificationUpdate.detail;
      const isAfterSendAt =
      ...
      if (isAfterSurveyDeadline && real == 0) {
        setFields({
          survey_deadline: {
      ...
      if (
        errors ||
        isErrorContent ||
        (isAfterSendAt && real == 0) ||
        (isAfterSendEventAt && real == 0) ||
        (isAfterSurveyDeadline && real == 0)
      ) {
        return;
      }
      ...
      const formData = {
        ...
        is_survey: this.state.is_survey ? 1 : 0,
        survey_deadline:
          this.state.is_survey &&
          values.survey_deadline.unix() > moment().add(1, "days").unix()
      ...
      if (status == 0) {
        ...
        dispatch(createNotificationUpdateReminder(formData));
        setTimeout(() => {
  ...
  onSelectChange = (selectedRowKeys, selectedRows) => {
    ...
    this.setState({
      ...
      apartment_not_send_ids: notificationUpdate.apartmentToSend.data
        .map((opt) => opt.apartment_id)
        .filter(
          (row, index, self) =>
            !new_apartment_ids.includes(row) && self.indexOf(row) === index
        ),
      resident_user_phones: notificationUpdate.apartmentToSend.data
  ...
  render() {
    const { notificationUpdate, intl, dispatch, language } = this.props;
    const { notiType, residentSearch, apartmentSearch, dataSource } =
      this.state;
    ...
    // const saveNotificationSuccessText = intl.formatMessage({
    //   ...messages.saveNotificationSuccess,
    // });
    const publicNotificationSuccess = intl.formatMessage({
      ...messages.publicNotificationSuccess,
    });
    ...
    const updateNotificationSuccessText = intl.formatMessage({
      ...messages.updateNotificationSuccess,
    });

    const { category, detail, loading, template, template_list } =
      notificationUpdate;
    ...
    if (!detail || detail.type != 0) {
      return (
        <Page inner>
          <Exception
            type="404"
            desc={<FormattedMessage {...messages.notFindPage} />}
            actions={
              <Button
                type="primary"
                onClick={() =>
              ...
              >
                <Row type="flex" justify="space-between">
                  ...
                  <Select
                    ...
                    placeholder={
                      <FormattedMessage
                        {...messages.chooseNotificationTemplate}
                      />
                    }
                    defaultValue={999}
                ...
                <Form labelAlign="left" {...formItemLayout}>
                  ...
                  {notiType === "6" && (
                    ...
                    >
                      {getFieldDecorator("survey_deadline", {
                        initialValue: this.state.survey_deadline
                          ? this.state.survey_deadline
                          : undefined,
                        rules: [
                          {
                            required: true,
                        ...
                        <DatePicker
                          showTime
                          style={{ width: "100%" }}
                          disabled={this.state.is_deadline}
                          placeholder={surveyDeadlinePlaceholderText}
                          format="HH:mm - DD/MM/YYYY"
                          // disabled={!canCreateOrUpdate || currentStep == 2}
                          disabledDate={(current) => {
                            if (detail.status == 1 && detail.is_survey) {
                              // Can only extend deadline to the future
                              return (
                                current &&
                                current <
                                  moment
                    ...
                    >
                      ...
                      {this.state.is_event &&
                        getFieldDecorator("send_event_at", {
                          initialValue: this.state.is_event
                            ? this.state.send_event_at
                            : moment().add(1, "days"),
                          rules: [
                            {
                              required: true,
                  ...
                  >
                    <Upload
                      disabled={
                        detail.status !== 0 ||
                        this.state.fileImageList.length >= 10
                      }
                  ...
                  >
                    <Upload
                      disabled={
                        detail.status !== 0 || this.state.fileList.length >= 5
                      }
                      showUploadList={true}
                      fileList={this.state.fileList}
                      accept={
                    ...
                    <Col md={12} xl={24}>
                      ...
                      >
                        {getFieldDecorator("pushType", {
                          initialValue: this.state.pushType,
                          rules: [
                            {
                              required: true,
                              message: (
                          ...
                          <Checkbox.Group>
                            <Checkbox
                              ...
                              style={{ marginTop: 8 }}
                              disabled={
                                detail.status !== 0 || this.state.is_survey
                              }
                            >
                            ...
                            {/* <Checkbox
                              value="is_send_sms"
                              disabled={
                                detail.status == 1 || this.state.is_survey
                              }
                            >
                      ...
                      >
                        <Row>
                          ...
                          <Col span={12}>
                            <Checkbox
                              disabled={detail.status !== 0}
                              checked={getFieldValue("targets").length == 4}
                              onChange={(value) => {
                      ...
                      <Col md={12} xl={24}>
                        ...
                        >
                          {getFieldDecorator("type_report", {
                            initialValue:
                              detail.type_report || this.state.formula,
                            rules: [
                              {
                                required: true,
                                message: (
                  ...
                  <Form.Item label={<FormattedMessage {...messages.sendTo} />}>
                    {getFieldDecorator("building_area_ids", {
                      ...
                      initialValue:
                        detail.status === 1
                          ? this.props.notificationUpdate.buildingArea.lst
                              .filter((item) =>
                                detail.building_area_ids.includes(item.id)
                              )
                              .map((item) => item.name) ||
                            detail.building_area_ids ||
                            []
                          : detail.building_area_ids,
                      rules: [
                        {
                  ...
                  {/* {getFieldValue("pushType") &&
                    getFieldValue("pushType").includes("is_send_sms") && (
                      <Form.Item
                        label={<FormattedMessage {...messages.smsContent} />}
                        colon={false}
                      >
                        {getFieldDecorator("content_sms", {
                          initialValue: detail.content_sms,
                          rules: [
                            {
                          ...
                          <Input.TextArea
                            onChange={(e) =>
                              this.setState({ content_sms: e.target.value })
                            }
                            rows={5}
                    ...
                    >
                      {getFieldDecorator("phone_list", {
                        ...
                        <div>
                          ...
                          {!inputPhoneVisible && detail.status == 0 && (
                            <Tag
                    ...
                    >
                      {getFieldDecorator("email_list", {
                        ...
                        <div>
                          ...
                          {!inputEmailVisible && detail.status == 0 && (
                            <Tag
                              className="site-tag-plus"
          ...
          <Col span={24} style={{ paddingTop: 24 }}>
            ...
            >
              <Col style={{ textAlign: "center" }}>
                ...
                >
                  {notificationUpdate.apartmentToSend.data.length || 0}
                </span>
              </Col>
              <Col style={{ textAlign: "center" }}>
                ...
                <span
                  style={{ fontWeight: "bold", fontSize: 28, lineHeight: 2 }}
                >
                  {notificationUpdate.apartmentToSend.data.filter(
            ...
            <Col span={24} style={{ marginTop: 24 }}>
              <Row type="flex" align="middle" justify="center">
                <Button
                  ...
                  style={{ marginRight: 8 }}
                  disabled={
                    notificationUpdate.apartmentToSend.totalPage == 0 ||
                ...
                <Button
                  ...
                  onClick={() => {
                    this.handleOk(1, publicNotificationSuccess);
                  }}
            ...
            <Col span={24} style={{ marginTop: 24 }}>
              <Row type="flex" align="middle" justify="center">
                <Button
                  type="primary"
                  size="large"
                  disabled={this.state.isLoadingCreate}
                  style={{ marginLeft: 8 }}
                  onClick={() => {
                    this.handleOk(2, updateNotificationSuccessText);
                  }}
                  loading={this.state.isLoadingCreate}
                >
