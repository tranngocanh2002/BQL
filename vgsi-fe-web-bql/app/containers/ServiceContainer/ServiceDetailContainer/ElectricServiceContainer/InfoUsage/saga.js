...
function* _fetchUsage(action) {
  try {
    let res = yield window.connection.fetchElectricInfoUsage({
      ...action.payload,
      pageSize: 20,
    });
    if (res.success) {
...
function* _importUsage(action) {
  let res = yield window.connection.importElectricInfoUsage({
    ...action.payload,
    is_validate: 0,
  });
...
function* _addInfo(action) {
  try {
    let res = yield window.connection.createElectricInfoUsage(action.payload);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
...
function* _updateInfo(action) {
  try {
    let res = yield window.connection.updateElectricInfoUsage(action.payload);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
...
function* _deleteInfo(action) {
  try {
    let res = yield window.connection.deleteElectricInfoUsage(action.payload);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
