...
function lockFeeTemplateElectricPageReducer(state = initialState, action) {
  switch (action.type) {
    case DEFAULT_ACTION:
      return initialState;
    ...
    case FETCH_DESCRIPTION_FEE_COMPLETE: {
      return state.setIn(['descriptionFee', 'loading'], false).setIn(['descriptionFee', 'data'], action.payload)
    }
    case CLEAR_CACHE_MODAL: {
      return state
        .setIn(['descriptionFee', 'loading'], true).setIn(['descriptionFee', 'data'], undefined)
        .set('lastMonthFee', {})
    }
    default:
      return state;
...
export default lockFeeTemplateElectricPageReducer;
