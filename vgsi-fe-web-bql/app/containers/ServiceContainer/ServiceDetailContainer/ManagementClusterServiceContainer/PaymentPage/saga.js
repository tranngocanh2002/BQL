...
import makeSelectManagementClusterServiceContainer, {
...
function* _createPayment(action) {
  try {
    let res = yield window.connection.createBuildingFee(action.payload);
...
function* _updatePayment(action) {
  try {
    let res = yield window.connection.updateBuildingFee(action.payload);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
