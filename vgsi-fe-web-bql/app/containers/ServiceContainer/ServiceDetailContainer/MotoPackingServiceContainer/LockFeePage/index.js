...
export class LockFeePage extends React.PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.props.lockFeePage.creating != nextProps.lockFeePage.creating &&
      nextProps.lockFeePage.success
    ) {
      this.setState({
        visible: false,
      });
    ...
    if (
      this.props.lockFeePage.importing != nextProps.lockFeePage.importing &&
      nextProps.lockFeePage.importingSuccess
    ) {
      this.setState({
        visible: false,
      });
      if (this.state.current == 1) {
        ...
        this.props.history.push(
          `/main/service/detail/moto-packing/lock?${queryString.stringify({
            ...this.state.filter,
            page: 1,
          })}`
        );
  ...
  render() {
    const { lockFeePage, dispatch, auth_group, intl } = this.props;
    ...
    return (
      <Row>
        ...
        <Table
          rowKey="id"
          loading={
            lockFeePage.loading || lockFeePage.deleting || lockFeePage.importing
          }
          columns={columns}
