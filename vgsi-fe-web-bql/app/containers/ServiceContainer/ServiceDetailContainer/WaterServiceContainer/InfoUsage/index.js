...
export class InfoUsage extends React.PureComponent {
    ...
    if (
      ...
      if (this.state.current == 1) {
        ...
        this.props.history.push(
          `/main/service/detail/water/usage?${queryString.stringify({
            ...this.state.filter,
            page: 1,
          })}`
        );
  ...
  _onDelete = (record) => {
    Modal.confirm({
      ...
      title: this.props.intl.formatMessage({ ...messages.confirmDeleteData }),
    ...
    confirm({
      ...
      onOk: () => {
        window.modalImport.show(
          (url) => {
            return window.connection.importWaterInfoUsage({
              file_path: url,
  ...
  render() {
    const { infoUsage, waterServiceContainer, auth_group, intl } = this.props;
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: "service_water_type_name",
        key: "service_water_type_name",
      },
      {
        align: "right",
        title: (
          <span className={styles.nameTable}>
            <FormattedMessage {...messages.endIndex} />
          </span>
        ),
        dataIndex: "end_index",
        key: "end_index",
            ...
            <DatePicker
              ...
              onChange={(value) => {
                this.setState(
                  ...
                  () => {
                    // this.props.history.push(`/main/service/detail/water/usage?${queryString.stringify({
                    //   ...this.state.filter,
                    //   page: 1,
                    // })}`)
                  }
              ...
              <Button
                onClick={() => {
                  this.setState(
                    ...
                    () => {
                      window.connection
                        .downloadWaterTemplateInfoUsage({})
                        .then((res) => {
