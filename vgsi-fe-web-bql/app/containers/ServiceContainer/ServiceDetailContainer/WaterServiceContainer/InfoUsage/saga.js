...
function* _fetchUsage(action) {
  try {
    let res = yield window.connection.fetchWaterInfoUsage({
      ...action.payload,
      pageSize: 20,
    });
    if (res.success) {
...
function* _importUsage(action) {
  let res = yield window.connection.importWaterInfoUsage({
    ...action.payload,
    is_validate: 0,
  });
...
function* _addInfo(action) {
  try {
    let res = yield window.connection.createWaterInfoUsage(action.payload);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
...
function* _updateInfo(action) {
  try {
    let res = yield window.connection.updateWaterInfoUsage(action.payload);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
...
function* _deleteInfo(action) {
  try {
    let res = yield window.connection.deleteWaterInfoUsage(action.payload);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
