...
function* _createPayment(action) {
  try {
    ...
    let res = yield window.connection.createWaterFee(rest);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (need_approve) {
        let resApprove = yield window.connection.approveWaterFee({
          is_active_all: 0,
...
function* _updatePayment(action) {
  try {
    const { need_approve, ...rest } = action.payload;
    let res = yield window.connection.updateWaterFee(rest);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (need_approve) {
        let resApprove = yield window.connection.approveWaterFee({
...
function* _fetchAllPayment(action) {
  try {
    let res = yield window.connection.fetchWaterFee({
      ...action.payload,
      pageSize: 20,
      status: 0,
    });
...
function* _deletePayment(action) {
  try {
    ...
    let res = yield window.connection.deleteWaterFee(rest);
    const language = yield select(makeSelectLocale());
    if (res.success) {
      if (language === "en") {
...
function* _importPayment(action) {
  try {
    let res = yield window.connection.importFeeWater({
      ...action.payload,
      is_validate: 0,
    });
    const language = yield select(makeSelectLocale());
...
function* _fetchDescriptionFee(action) {
  try {
    let res = yield window.connection.fetchDescriptionFee({
      ...action.payload,
    });
    if (res.success) {
...
function* _approvePayment(action) {
  try {
    const { callback, ...rest } = action.payload;
    let res = yield window.connection.approveWaterFee({
      ...rest,
    });
    const language = yield select(makeSelectLocale());
    if (res.success) {
