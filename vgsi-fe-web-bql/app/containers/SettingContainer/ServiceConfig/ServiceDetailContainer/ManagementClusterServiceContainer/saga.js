...
function* _fetchDetailService(action) {
  try {
    let res = yield Promise.all([
      ...
      window.connection.fetchDetailServiceBuildingConfig({ service_base_url: action.payload }),
    ])
    if (res[0].success && res[0].data.items.length == 1) {
      if (!!!res[1].data) {
        let resCreateDetail = yield window.connection.createServiceBuildingConfig({
          price: 3000,
          auto_create_fee: 1,
          unit: 0,
          day: 1,
