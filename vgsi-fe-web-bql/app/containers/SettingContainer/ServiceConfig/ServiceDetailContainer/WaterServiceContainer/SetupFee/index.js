...
import makeSelectSetupFeeWaterPage from "./selectors";
import("./index.less");
...
@Form.create()
export class SetupFeeWaterPage extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return (
            <span>{`${this.props.intl.formatMessage(messages.dinhMuc)} ${
              index + 1
            }`}</span>
          );
        },
      },
      {
        title: (
          <span className="nameTable">
            {this.props.intl.formatMessage(messages.from)} (m3)
          </span>
        ),
        dataIndex: "from_level",
        editable: true,
      ...
      },
      {
        title: (
          <span className="nameTable">
            {this.props.intl.formatMessage(messages.to)} (m3)
          </span>
        ),
        dataIndex: "to_level",
        editable: true,
      },
      {
        ...
        title: (
          <span className="nameTable">
            {this.props.intl.formatMessage(messages.price)} (đ/m3)
          </span>
        ),
        dataIndex: "price",
        editable: true,
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.SetupFeeWaterPage.success !=
        nextProps.SetupFeeWaterPage.success &&
      nextProps.SetupFeeWaterPage.success
    ) {
      this.setState({
        editingKey: "",
        data: nextProps.SetupFeeWaterPage.items,
      });
    }
    if (
      this.props.SetupFeeWaterPage.loading !=
        nextProps.SetupFeeWaterPage.loading &&
      !nextProps.SetupFeeWaterPage.loading
    ) {
      this.setState({
        editingKey: "",
    ...
    form.validateFields((error, row) => {
      ...
      if (record.isNew) {
        this.props.dispatch(
          createWaterFeeLevel({
            ...row,
            name: `${this.props.intl.formatMessage(messages.dinhMuc)}: ${
              index + 1
            }`,
            name_en: `Level: ${index + 1}`,
            description: "description",
            service_map_management_id: this.props.waterServiceContainer.data.id,
            service_id: this.props.waterServiceContainer.data.service_id,
  ...
  render() {
    ...
    const { SetupFeeWaterPage } = this.props;
      ...
      <Row className="setupFee">
        ...
        <EditableContext.Provider value={this.props.form}>
          <Table
            ...
            pagination={false}
            loading={SetupFeeWaterPage.loading || SetupFeeWaterPage.updating}
          />
...
SetupFeeWaterPage.propTypes = {
  dispatch: PropTypes.func.isRequired,
};

const mapStateToProps = createStructuredSelector({
  SetupFeeWaterPage: makeSelectSetupFeeWaterPage(),
...
const withReducer = injectReducer({ key: "SetupFeeWaterPage", reducer });
const withSaga = injectSaga({ key: "SetupFeeWaterPage", saga });

export default compose(
  withReducer,
  withSaga,
  withConnect
)(injectIntl(SetupFeeWaterPage));
